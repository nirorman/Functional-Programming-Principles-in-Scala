 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("max negative numbers") {
    assert(max(List(-1, -3, -100)) === -1)
  }

  test("max negative ,0 , positive numbers") {
    assert(max(List(-10, 0, 10)) === 10)
  }
  ...
  test("sum negative numbers") {
    assert(sum(List(-1, -3, -100)) === -104)
  }

  test("sum negative ,0 , positive numbers") {
    assert(sum(List(-7, 0, 10)) === 3)
  }
